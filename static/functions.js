...
function sendMessage() {
  ...
  if (comment !== "") {
    socket.send(
      JSON.stringify({ messageType: "chatMessage", comment: comment })
    );
  }
}
  ...
  switch (messageType) {
    ...
    case "webRTC-offer":
      ...
      webRTCConnection.createAnswer().then((answer) => {
        ...
        socket.send(
          JSON.stringify({ messageType: "webRTC-answer", answer: answer })
        );
      });
      break;
    case "webRTC-answer":
...
function connectWebRTC() {
  ...
  webRTCConnection.createOffer().then((webRTCOffer) => {
    socket.send(
      JSON.stringify({ messageType: "webRTC-offer", offer: webRTCOffer })
    );
